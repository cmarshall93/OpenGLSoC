...
public class Pathfinder{
	 ...
	 */
	public Pathfinder(int maxSearchDistance, boolean allowDiagMovement) {
	 ...
	 * @param allowDiagMovement True if the search should try diaganol movement
	 */
	public Pathfinder(int maxSearchDistance, 
						   boolean allowDiagMovement, AStarHeuristic heuristic) {
		this.heuristic = heuristic;
		this.maxSearchDistance = maxSearchDistance;
		this.allowDiagMovement = allowDiagMovement;
		
		nodes = new Node[GameModeState.getInstance().getMap().length][GameModeState.getInstance().getMap().length];
		for (int x=0;x<GameModeState.getInstance().getMap().length;x++) {
			for (int y=0;y<GameModeState.getInstance().getMap().length;y++) {
	...
	protected boolean isValidLocation(int sx, int sy, int x, int y) {
		boolean invalid = (x < 0) || (y < 0) || (x >= GameModeState.getInstance().getMap().length) || 
	...
	public float getHeuristicCost(int x, int y, int tx, int ty) {
		float dx = (float) Math.pow((tx - x), 2);
		float dy = (float) Math.pow((ty - y), 2);
